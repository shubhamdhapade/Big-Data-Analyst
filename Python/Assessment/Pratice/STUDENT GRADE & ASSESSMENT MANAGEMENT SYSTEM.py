...
def compute_grade(marks):
    """Calculates letter grade based on numerical marks."""
    if marks >= 85.0:
        return "A"
    elif marks >= 70.0:
        return "B"
...
def get_next_id():
    """Generates a unique incremental ID for new records."""
    if not students:
        return 1
    ...
    if new_marks_str:
        try:
            ...
            if 0.0 <= new_marks <= 100.0:
                ...
                record["grade"] = compute_grade(new_marks)
            else:
...
def load_from_json(filename="students.json", verbose=True):
    """Module C: JSON Deserialization with duplicate prevention via unique 'id' tracking."""
    global students
    if not os.path.exists(filename):
        if verbose:
            print(f"Warning: '{filename}' not found. Starting with empty catalog.")
        return
    try:
                ...
                for record in data:
                    if isinstance(record, dict) and "id" in record:
                        if record["id"] not in existing_ids:
                            ...
                            added_count += 1
                if verbose:
                    print(f"Data loaded from '{filename}'. Imported {added_count} new record(s). Total in catalog: {len(students)}")
            else:
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            confirm = input("\nDo you want to save/update all records to the JSON file before exiting? (y/n): ").strip().lower()
            if confirm == 'y':
                save_to_json()
            else:
                print("Exiting without saving recent changes.")
