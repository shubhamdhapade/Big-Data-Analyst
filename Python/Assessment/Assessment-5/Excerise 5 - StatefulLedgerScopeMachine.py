"""
    ...
    Scenario
    You are developing a stateful balance ledger tracker that tracks account state history. To satisfy strict architecture 
    requirements, you must manage this state without defining any classes (class keyword is prohibited). Instead, you must use 
    closures, nested functions, and Python scoping variables.
    ...
    Inside create_bank_account, initialize local variables balance (float, set to initial_balance) and history (list of strings, 
    initially containing ["Account created with 1000.0"] or similar initial message).
    ...
    ≥
    amount).
    If yes, deducts amount from balance, appends the string "withdraw <amount>" to history, and increments global 
    AUDIT_TRANSACTION_COUNT by 1.
    If the balance is insufficient, raises a standard ValueError with message "Insufficient balance".
    get_statement():
    Returns a tuple containing (owner_name, current_balance, history_list_copy). (Note: Make sure history_list_copy is a copy of 
    the history list to prevent direct external modification).
...
def create_bank_account(owner_name, initial_balance):
    ...
    def deposit(amount):
        ...
        history.append(f"deposit {float(amount)}")
    ...
    def withdraw(amount):
        ...
        balance -= amount_f
        history.append(f"withdraw {amount_f}")
        AUDIT_TRANSACTION_COUNT += 1
...
if __name__ == "__main__":
    print(f"Initial global count: {AUDIT_TRANSACTION_COUNT}") # Output: 0
    ...
    print(f"Owner: {owner}")
    print(f"Balance: {bal}")
    print(f"History: {txn_history}")

    print(f"Final global count: {AUDIT_TRANSACTION_COUNT}")
